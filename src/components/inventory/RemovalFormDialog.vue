  ...
  setup(props, { emit }) {
    ...
    const isValidQuantities = computed(() => {
      return selectedIngredients.value.every((ingredient) => {
        ...
        const roundedQuantity = parseFloat(quantity.toFixed(2)) // Round entered quantity to 2 decimal places
        const roundedCurrentQuantity = parseFloat(ingredient.current_quantity.toFixed(2)) // Round current quantity to 2 decimal places
        return (
          roundedQuantity !== undefined &&
          roundedQuantity > 0 &&
          roundedQuantity <= roundedCurrentQuantity
        )
      })
    })
    ...
    const fetchIngredients = async () => {
      ...
      try {
        const allIngredients = await db.getAll<'ingredients'>('ingredients')
        ingredients.value = allIngredients
          .filter((ingredient: Ingredient) => ingredient.current_quantity > 0) // Only show ingredients with stock
    ...
    const submitRemoval = async () => {
      ...
      try {
        ...
        const removalData = await db.insert('removals', {
          reason: reason.value,
        ...
        if (error instanceof Error) {
          console.error('Error submitting removal:', error.message)
          showError(
