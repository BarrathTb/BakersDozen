<template>
  <v-dialog v-model="showDialog" persistent max-width="500">
    <v-card>
      ...
      <v-card-text>
        <p class="mb-4">
          We've detected an issue with your authentication session. This might cause problems with
          accessing protected areas of the application or result in a blank screen after refresh.
        </p>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="success" @click="attemptAutoRecovery" :loading="isRecovering">
          Auto-Recover
        </v-btn>
        <v-btn color="primary" text @click="dismissDialog"> Dismiss </v-btn>
        <v-btn color="warning" @click="clearSessionAndReload" :loading="isClearing">
          Sign Out & Reload
        </v-btn>
      </v-card-actions>
    </v-card>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const registerAuthError = (error: { message?: string }) => {
